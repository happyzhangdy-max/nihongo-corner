...
(function (root) {
  'use strict';
  const kinds = [['topic', '主题词库'], ['special', '语言专项']];
  const descriptions = {
    topic: '按语义整理词汇，用例句了解词的用法。',
    special: '按语言特点与练习用途整理，适合有针对性地复习。'
  };
  const escape = value => String(value).replace(/[&<>"']/g, c => ({'&': '&amp;', '<': '&lt;', '>': '&gt;', '"': '&quot;', "'": '&#39;'}[c]));
  function mount(container, {vocab, selected, kind = 'topic', onChange, onKindChange}) {
    const catalog = root.VocabularyCatalog;
    let selection = selected.slice();
    let current = kinds.some(item => item[0] === kind) ? kind : 'topic';
    let query = '';
    ...
    container.innerHTML = '<div class="catalog-tabs" role="group" aria-label="分类方式">' + kinds.map(([key, name]) => '<button type="button" data-catalog-kind="' + key + '" aria-pressed="' + (key === current) + '">' + name + '<small>' + lists[key].length + '</small></button>').join('') + '</div><p class="study-note" id="catalogDescription"></p>' +
      '<label class="study-search-label" for="studyCategorySearch">查找分类或词汇</label><input id="studyCategorySearch" class="study-search" type="search" placeholder="例如：食物、交通、電車" autocomplete="off">' +
      ...
      '<div class="catalog-toolbar"><span id="catalogResultCount"></span><button type="button" data-catalog-action="select-visible" id="catalogSelectVisible"></button></div><div id="catalogGroups"></div><p class="study-note" id="studyNoCategories" role="status" hidden>没有匹配结果，请试试主题名、日文词或中文释义。</p><div id="catalogPreview"></div>';
    function renderGroups() {
      ...
      container.querySelector('#catalogResultCount').textContent = visible.length + ' 个' + '分类' + ' · 可多选';
    ...
    function renderSelection() {
      ...
      container.querySelector('#studySelectedCategories').textContent = selection.length ? '已选 ' + selection.length + ' 类 · 共 ' + words.length + ' 词' : '选择一个主题或专项开始';
    ...
    container.onclick = event => {
      ...
      if (button.dataset.catalogId) {
        ...
        selection = selection.includes(id) ? selection.filter(value => value !== id) : [...selection.filter(value => value !== 'legacy:all'), id];
        changed(); return;
      }
      ...
      if (button.dataset.catalogAction === 'select-visible') {
        selection = [...new Set([...selection.filter(value => value !== 'legacy:all'), ...lists[current].filter(item => item.count && item.search.includes(query)).map(item => item.id)])]; changed();
      }
    };
