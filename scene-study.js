...
(function (root) {
  ...
  function renderLesson() {
    ...
    page().innerHTML = '<div class="scene-study"><button type="button" class="scene-text-button" data-scene-action="directory">← 全部场景</button>' + heading(scene.label, scene.description) +
      '<p class="scene-scope">本场景 ' + scene.dialogues.length + ' 组对话 · ' + scene.dialogues.length * 2 + ' 句；只学习这个场景。</p>' +
      '<label class="scene-task-select">交流任务<select id="sceneTaskSelect"><option value="">全部任务（' + scene.dialogues.length + ' 组对话）</option>' + scene.dialogues.map(d => '<option value="' + escape(d.id) + '"' + (taskId === d.id ? ' selected' : '') + '>' + escape(d.task) + '</option>').join('') + '</select></label>' +
